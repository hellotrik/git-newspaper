...
import { detectArchetype, ARCHETYPE_META } from './archetype.js'
...
export function buildContent(data) {
  ...
  return {
    masthead:    buildMasthead(data, archetype),
    headline:    buildHeadline(data, archetype, rng),
    lead:        buildLead(data, archetype, rng),
    weather:     buildWeather(data, archetype, rng),
    obituaries:  buildObituaries(data, rng),
    charts:      buildCharts(data),
    opinion:     buildOpinion(data, archetype, rng),
    stats:       buildStats(data, archetype),
    classifieds: buildClassifieds(data, archetype, rng),
...
function buildMasthead(data, archetype) {
  const d = data.lastCommit?.date ?? new Date()
  return {
    name:    'The Git Gazette',
    repo:    data.repoName,
    tagline: 'All the commits fit to print.',
    date:    format(d),
    edition: ARCHETYPE_META[archetype].label,
    price:   'FREE',
  }
}
...
function buildHeadline(data, archetype, rng) {
  if (!data.biggestCommit) return {
    kicker: 'INAUGURAL ISSUE', headline: 'REPOSITORY ESTABLISHED', deck: '', byline: 'Staff', date: format(new Date()), hash: 'unknown',
  }

  const c      = data.biggestCommit
  const clean  = c.subject.replace(/^(feat|fix|chore|refactor|docs|test|perf|style|ci|build)(\(.+?\))?:\s*/i, '')
  const opener = rng.pick(HEADLINE_OPENERS[archetype])
  const qual   = rng.pick(HEADLINE_QUALIFIERS[archetype])
  const hed    = [opener, titleCase(clean)].filter(Boolean).join(' ') + qual

  return {
    kicker:  classifyKicker(c.subject, archetype),
    headline: hed,
    deck:    buildDeck(c, data.repoName, archetype, rng),
    byline:  c.author,
    date:    format(c.date),
    hash:    c.hash,
  }
...
function buildDeck(commit, repoName, archetype, rng) {
  const templates = DECK_TEMPLATES[archetype] ?? DECK_TEMPLATES.BALANCED
...
function buildLead(data, archetype, rng) {
  ...
  const span    = firstCommit && lastCommit
    ? `from ${format(firstCommit.date)} to ${format(lastCommit.date)}`
    : 'across the recorded period'
  const nAuthors = Object.keys(allAuthors).length

  const opener = rng.pick(LEAD_OPENERS[archetype] ?? LEAD_OPENERS.BALANCED)(c, repoName, totalCommits, allAuthors)
  const middle = rng.pick(LEAD_MIDDLES)(topFile, c)
  const closer = rng.pick(LEAD_CLOSERS)(totalCommits, span, nAuthors)
...
function buildWeather(data, archetype, rng) {
  return rng.pick(WEATHER_CONDITIONS[archetype] ?? WEATHER_CONDITIONS.BALANCED)
}
...
function buildObituaries({ deletedFiles, repoName }, rng) {
  const churnMap = {}
  return deletedFiles.slice(0, 4).map(({ file, commit }) => {
    const name     = file.split(/[/\\]/).pop()
    const dir      = file.includes('/') || file.includes('\\')
      ? file.replace(/[^/\\]+$/, '').slice(0, -1)
      : repoName
    const category = classifyDeletedFile(file, churnMap)
    const pool     = EPITAPHS[category] ?? EPITAPHS.normal
    return {
      name,
      path:    file,
      survived: `committed by ${commit.author}`,
...
function buildOpinion({ mostChurned, repoName }, archetype, rng) {
  if (!mostChurned.length) return null
  const top   = mostChurned[0]
  const name  = top.file.split(/[/\\]/).pop()
  const churn = top.churn
  const body  = rng.pick(OPINION_BODIES)(name, churn, repoName)

  return {
    columnist: name,
    title:     rng.pick([
      `"${churn} Changes and Nobody Asked Me Once"`,
      `"I Am the Most-Modified File in This Repository and I Have Thoughts"`,
      `"A Request for Refactoring, With Supporting Evidence"`,
      `"On Being Called 'Temporary' in April: A Retrospective"`,
    ]),
    subtitle: `An opinion by ${name}, as told to The Git Gazette`,
    body,
  }
}
...
function buildStats(data, archetype) {
  const types = {}
  ...
  return {
    ...
    avgPerCommit:    data.totalCommits ? Math.round((data.totalInsertions + data.totalDeletions) / data.totalCommits) : 0,
...
function buildClassifieds(data, archetype, rng) {
  const pool   = [...(CLASSIFIED_POOLS[archetype] ?? []), ...CLASSIFIED_POOLS.BALANCED]
...
function buildCharts({ commits }) {
  return { timeline: buildTimeline(commits) }
}
...
function buildTimeline(commits) {
  ...
  return {
    ...
    peakIdx,
    peakCount: points[peakIdx]?.count ?? 0,
  }
}

// ── Helpers ───────────────────────────────────────────────────────────────────

function classifyKicker(subject, archetype) {
  const TYPE_KICKER = {
    feat: 'FEATURE', fix: 'BUGFIX', refactor: 'REFACTOR',
    revert: 'REVERSION', docs: 'DOCUMENTATION', test: 'TESTING',
    perf: 'PERFORMANCE', chore: 'ADMINISTRATION', ci: 'INFRASTRUCTURE',
  }
  const m = subject.match(/^(\w+)(\(.+?\))?:/)
  if (m && TYPE_KICKER[m[1].toLowerCase()]) return TYPE_KICKER[m[1].toLowerCase()]

  const ARCH_KICKER = {
    REVERT_CRISIS: 'CRISIS', SOLO_MARATHON: 'PROFILE', GHOST_TOWN: 'RARE DEVELOPMENT',
    DEPENDENCY_CHURN: 'ADMINISTRATION', REFACTOR_SWEEP: 'INFRASTRUCTURE',
    BUGFIX_CRISIS: 'EMERGENCY', FEATURE_SPRINT: 'DEVELOPMENT', BALANCED: 'DEVELOPMENT',
  }
  return ARCH_KICKER[archetype] ?? 'DEVELOPMENT'
}
...
function commitType(subject) {
  ...
  if (/^test/i.test(subject))     return 'Tests'
